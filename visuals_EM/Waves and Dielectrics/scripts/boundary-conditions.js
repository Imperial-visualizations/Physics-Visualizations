$(window).on('load', function() {
    ...
    function init(data) {
        ...
        traces.push({name: "Incident E", x: deepCopy(phys.getPlotData())[2].z,y:deepCopy(phys.getPlotData())[2].y,
        mode: "lines",line: {width: 3}})
        traces.push({name: "Transmitted E",x: deepCopy(phys.getPlotData())[6].z,y:deepCopy(phys.getPlotData())[6].y,
        mode: "lines",line: {width: 3}})
        traces.push({name: "Reflected E",x: deepCopy(phys.getPlotData())[10].z,y:deepCopy(phys.getPlotData())[10].y,
        mode: "lines",line: {width: 3}})
        var total_x = [],
            total_y= [];
        for (var i = 0; i < deepCopy(phys.getPlotData())[2].z.length; i++) {
            total_x.push(deepCopy(phys.getPlotData())[2].z[i]);
            total_y.push(deepCopy(phys.getPlotData())[2].y[i]+deepCopy(phys.getPlotData())[10].y[i]);
        }
        traces.push({name: "Total E on right side",x: total_x,y:total_y,
