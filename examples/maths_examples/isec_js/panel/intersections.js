...
function Point(pos) {
	...
	function intersectList(objlist) {
		...
		for(var idx = 0; idx<objlist.length;idx++) {
			for(var other = idx+1; other<objlist.length;other++) {
				...
				try {
					...
					//objlist.push(toPush); //this bit makes deep search possible
					ans.push(toPush);
				}
				catch(err) {
					if(!(err instanceof NoIntersectionError)) {
						// ignore no intersection errors
						throw err;
					}
				}
			}
...
function Point(pos) {
	...
	function intersectList(array) {
	  ...
	  function getGen(a) {
	    ...
	    ans = {'points': points, 'lines': lines, 'planes': planes};
	    return ans;
	  }
