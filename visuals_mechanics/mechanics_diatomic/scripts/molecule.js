/** ============================================ Class Declarations ==============================================*/
    ...
    Atom = function(radius, mass, potential, color) {
        ...
        this.v = 0;                                 // Momentum of atom initially 0.
        this.f_net = 0;                             // Net force on atom - initially 0.
    };
    ...
    Molecule = function(atoms, potential, vibrational_E, rotational_E) {
        ...
        if (atoms.constructor === Array || atoms[0].constructor === Atom)
        {
            ...
            this.r = this.atoms[0].pos.subtract(this.atoms[1].pos).mag();     // Atom separation.
    ...
    Molecule.prototype.calcPE = function() {
        var r = this.calcSeparation(this.atoms[0], this.atoms[1]);          // Distance between atoms.
        this.PE = this.V.calcV(r);                                          // Potential energy calculation.
        return this.PE
    };
    /**
    ...
    Molecule.prototype.calcRotCoords = function(t) {
        ...
        this.w = Math.sqrt(2 * this.E_r / this.I);                           // Angular velocity, w.
        var d_Angle = this.w * t;                                            // Rotation angle around COM (radians).

        // Rotation of atoms about COM.
        for (var i = 0; i < this.atoms.length; i++) {
            // Find new atom coords after rotation around COM.
    ...
    Molecule.prototype.calcExtCoords = function (dT) {
        ...
        for (var i = 0; i < this.atoms.length; i++) {
            for (var j = 0; j < this.atoms.length; j++) {
                if (i !== j) {
                    var r_s = this.calcSeparation(this.atoms[i], this.atoms[j]);              // Distance to other atom.
                    var k_i = new Vector(this.calcUnitVect(this.atoms[i], this.atoms[j]));    // Unit vector

                    this.atoms[j].f_net = this.V.calcF(r_s);                             // |F| on atom due to other atom.

                    // Centrifugal displacement.
                    var r_cf = this.atoms[j].f_net / (this.reducedM() * Math.pow(this.w, 2));
                    var r_cf_v = new Vector(k_i.multiply(-1 * r_cf));

                    var a_s = this.atoms[j].f / this.atoms[j].m;                     // Scalar acceleration.
                    this.atoms[j].v = this.atoms[j].v + (a_s * dT);                  // v = u + at

                    // Calculating vector displacement.
                    var displacement = new Vector(new Vector(k_i.multiply(this.atoms[j].v)).multiply(dT));
                    s.push(displacement.add(r_cf_v))
                }
            }
        }

        for (var k = 0; k < s.length; k++) {
            this.atoms[k].pos = s[k].add(this.atoms[k].pos);                         // Updating atom coordinates.
        }
    };
